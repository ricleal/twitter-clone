...
- `internal/service/repository/postgres/models`: ORM code auto-generated from the database schema by [bob](https://github.com/stephenafamo/bob).
- `internal/service/repository/postgres/models/factory`: test factory helpers generated by bob for creating model instances in tests.
- `internal/service/repository/postgres/dbinfo`: database schema metadata generated by bob.
- `internal/service/repository/postgres/dberrors`: database error helpers generated by bob.
...
- `internal/service/repository/memory`: a memory mock repository that implements the interfaces defined in `internal/service/repository/interfaces.go`. This is used for unit testing.
...
Routes and types are generated from [openapi.yaml](openapi.yaml) using [oapi-codegen](https://github.com/deepmap/oapi-codegen):
...
Models are generated from the live database schema using [bobgen-psql](https://bob.stephenafamo.com/docs/code-generation/psql) (configured in [bobgen.yaml](bobgen.yaml)):
...
If you have one, edit the `.env` file (copied from the `env-template`) to point to your database. The database runs in an isolated Docker bridge network (`twitter-clone-db`) and binds only to `127.0.0.1` to avoid conflicts with other local databases.
