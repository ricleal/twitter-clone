...
var (
	userAllColumns            = []string{"id", "username", "email", "name", "deleted_at", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"id", "username", "email"}
	userColumnsWithDefault    = []string{"name", "deleted_at", "created_at", "updated_at"}
...
func (userL) LoadTweets(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`tweets`),
		qm.WhereIn(`tweets.user_id in ?`, args...),
		qmhelper.WhereIsNull(`tweets.deleted_at`),
	)
	if mods != nil {
		mods.Apply(query)
	}
